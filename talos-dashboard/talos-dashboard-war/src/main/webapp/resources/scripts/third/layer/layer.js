...
window.layer = {
    ...
    confirm: function(msg, yes, fn, no){ 
        var isfn = (typeof fn === 'function'), conf = {
            dialog: {msg: msg, type: -1, btns: 2, yes: yes, no: isfn ? fn : no}
        };
        isfn || (conf.title = fn);
        return $.layer(conf); 
    },
...
Class.pt.set = function(times){
    ...
    switch(config.type){
        ...
        case 4:
            ...
            layerE.outerWidth() < config.minWidth && layerE.width(config.minWidth);
...
Class.pt.autoArea = function(times){
    ...
    if(config.area[0] === 'auto' && layerMian.outerWidth() <= config.minWidth){	
        layerE.css({width : config.minWidth});
    }
